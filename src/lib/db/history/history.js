    ...
    try {
        const result = await pool.request()
                                 ...
                                 .query(`
                                        ...
                                        LEFT JOIN dbo.storage_stocks AS b ON a.stock_in_id = b.id
                                        RIGHT JOIN dbo.item_master AS e ON b.item_id = e.id 
                                        ...
                                        SELECT  
                                                ...
                                                a.modified_datetime
                                        FROM dbo.transaction_history AS a 
                                        LEFT JOIN dbo.[order] AS b ON a.order_id = b.id
                                        RIGHT JOIN dbo.item_master AS e ON b.item_id = e.id 
                                        WHERE CAST(a.transaction_date AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(@endDate AS DATE)

                                        UNION ALL

                                        -- Third part: lounge stocks
                                        SELECT 
                                                a.id,
                                                ...
                                                a.modified_datetime
                                        FROM dbo.transaction_history AS a 
                                        LEFT JOIN dbo.lounge_stocks AS b ON a.lounge_id = b.id
                                        RIGHT JOIN dbo.item_master AS e ON b.item_id = e.id 
                                        WHERE CAST(a.transaction_date AS DATE) BETWEEN CAST(@startDate AS DATE) AND CAST(@endDate AS DATE)
                                        ), ROWNUM AS (
                                                SELECT id, source_id, item_name, kode_sku, transaction_date, transaction_type, transaction_qty, before_qty, storage_qty, order_qty, lounge_qty,
                                                        created_by, created_datetime, modified_by, modified_datetime, 
                                                ROW_NUMBER() OVER (PARTITION BY kode_sku ORDER BY transaction_date DESC) AS rn
                                                FROM Transactions
                                        )
